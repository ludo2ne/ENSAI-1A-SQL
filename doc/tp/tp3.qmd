...
Pour plus de détails, vous pouvez consulter la section [Pour aller plus loin](#pour-aller-plus-loin).

### Les services {.unnumbered}

Le principe est d'offrir aux utilisateurs la possibilité de lancer de nombreux services à la demande (Jupyter, Rstudio, VSCode, PostgreSQL...) et une puissance de calcul adaptée aux besoins.
...
Pour retrouver les 5 chiffres du numéro du dépanneur, vous allez devoir répondre à 5 questions en requêtant fichier des communes françaises.
:::
...
Ce sont des commandes pour interragir avec voter stockage depuis un termial (voir ci-après).
...
Vous venez de charger un fichier, voyons maintenant comment y accéder dans un *Service*, par exemple le notebook que vous aviez lancé.
...
- [ ] Créer 2 cellules, puis collez et exécutez les 2 blocs de code ci-dessous
  - Utilisez par exemple le bouton `+` de la barre d'outils pour créer une cellule
...
- [ ] Créez une 3e cellule et affichez le fichier chargé
  - Que se passe-t-il ?

::: {.callout-note title="Mode Lazy" collapse="true"}
Vous avez appelé la méthode `scan_parquet()` qui correspond au mode *Lazy* de Polars.

Le mode *Lazy* signifie que Polars va accumuler toutes les instructions que vous lui donner sans pour autant les exécuter tout de suite.

Mais quel est l'intérêt ?

Imaginons que vous analisez un trés gros fichier et que vous enchainez à la suite diverses opérations (filtres, sélections de colonnes, regroupements, agrégations, tri...), deux choix s'offrent à vous :

Option 1 (mode Eager : `pl.read_parquet()`):

- vous chargez tout le fichier en mémoire
...
- à ce moment, Polars va optimiser l'ordre des traitement en utilisant son propre [plan d'exécution]{.underline} et vous donner une réponse beaucoup plus rapide
...
- [ ] Pour forcer le chargement du fichier, appelez la méthode `collect()`
  - stockez le résultat dans une variable *df*
- [ ] Vous pouvez maintenant afficher le contenu de *df* avec un print
- [ ] Filtrez le fichier pour trouver la longueur de fil du gilet
  - `filter(pl.col("<column_name>") == "<value>")`
...
Or Josette n'a finalement plus faim. Félix se met alors à bouder et va s'enfermer dans les toilettes pour avaler des pilules qu'il a pris au passage à la pharmacie. Soudain la mémoire lui revient !
...
- Datalab du GENES : `s3/ldeneuville-ensai/diffusion/ENSAI/TP-SQL/prenoms-nat2022.parquet`
- SSP Cloud : `s3/ludo2ne/diffusion/ENSAI/TP-SQL/prenoms-nat2022.parquet`
...
::: {.callout-important icon="false" title="Fil rouge"}
Alors que vous alliez chercher le prénom, votre cammarade Katia vous demande en quelle année son prénom a été le plus donné entre 1960 et 1980.
:::

- [ ] Écrivez la requête SQL qui fait le même travail que ces instructions
...
- [ ] Écrivez la requête
  ...
  - Classez-les par le nombre de fois où ils ont été donné
  - Créez une colonne de rang avec `RANK() OVER (ORDER BY SUM(nombre) DESC) AS rang`
- [ ] Trouvez le prénom classé au rang demandé

::: {.callout-tip collapse="true"}
- Vous pouvez stocker le résultat de la 1ère requête dans une vue
- Puis effectuer une recherche sur le rang

La réponse commence par un *S*
:::
...
- [ ] En utilisant la librairie *s3fs*
  ...
  destination = f"s3://{username}/ENSAI/SQL/output.csv"
  
  with fs.open(destination, mode='wb') as f:
      top10f2021.write_csv(f)
  ```
- [ ] Sur le Datalab, allez dans Mes fichiers :arrow_right: diffusion :arrow_right: ENSAI
...
Contactez l'équipe du SSP Lab sur [Slack](https://join.slack.com/t/3innovation/shared_invite/zt-1bo6y53oy-Y~zKzR2SRg37pq5oYgiPuA){target="_blank"}
