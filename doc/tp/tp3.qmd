...
Vous pouvez réaliser ce TP soit sur le :
...
Vous retrouvez la recette mais il vous manque la quantité exacte en grammes de magarine.

Vous savez simplement que la valeur recherchée correspond à combien il y a de nombres premiers entre 10 000 et 100 000.

Naturellement, vous vous dites que la solution la plus simple est de créer un programme Python pour trouver la réponse. Or Python n'est pas installé sur votre machine et vous ne savez pas comment faire... Et soudain, vous avez une illumination : pourquoi ne pas lancer un service du Datalab qui permet d'exéctuer du Python.
:::
...
Comme vous êtes un as de la programmation, cela ne vous prend que quelques minutes. C'est beaucoup plus rapide que si vous aviez du installer Python ainsi qu'un environnement d'exécution.

- [ ] Vous stockerez ce résultat dans une variable `a`
- [ ] Vérifiez que `a` est lui même un nombre premier


### Utiliser des données

::: {.callout-important icon="false" title="Fil rouge"}
Vous vous rendez compte qu'il faut beaucoup de margarine et que vous n'en avez pas assez dans le frigo.

Félix propose d'aller en chercher à la superette du coin. Il entre dans l'ascenseur pour descendre les 6 étages et patatra, il se retrouve bloquer.

Pour retrouver le numéro du dépanneur, vous allez devoir retrouver les chiffres en requêtant le fichier des prénoms
:::


:construction: appel via DuckDB


:construction: afficher un graph


## Stockage S3

::: {.callout-important icon="false" title="Fil rouge"}
Maintenant que Josette est repue, elle se souvient qu'une information importante se trouve dans le fichier des prénoms
:::

:construction: Aller chercher une info dans un fichier sur ludo2ne/diffusion (SSPLab)
