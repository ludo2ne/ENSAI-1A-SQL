...
Lors du premier TP, nous avons utilisé DuckDB pour lire les données d'un fichier parquet. Aujourd'hui nous allons créer notre propre base de données PostgreSQL et y insérer des tables et de données. Enfin nous allons requêter ces données.
...
- [ ] Lancez le service [PostgreSQL](https://onyxia.lab.groupe-genes.fr/launcher/databases/postgresql?name=postgresql&version=1.0.3&autoLaunch=true&postgresql.primary.persistence.size=«1Gi»&postgresql.primary.resources.limits.cpu=«250m»&postgresql.primary.resources.requests.cpu=«50m»&postgresql.primary.resources.limits.memory=«1Gi»){target="_blank"}
...
Nous avons créé un SGBD, nous avons maintenant besoin d'un outil de gestion pour s'y connecter et envoyer des requêtes.

- [ ] Lancez le service [CloudBeaver](https://onyxia.lab.groupe-genes.fr/launcher/databases/cloudbeaver?name=cloudbeaver&version=2.0.3&resources.requests.memory=%C2%AB1Gi%C2%BB&resources.limits.memory=%C2%AB2Gi%C2%BB){target="_blank"}
  - Allez dans **Catalogue de services**
  - Onglet **Databases**
...
Dans le cas contraire, il faut cliquer sur le +, puis sur *New connection* et renseigné les éléments de connexion.
...
- [ ] À la date du 10 aout 2010, combien de DVD sont en cours d'emprunt
