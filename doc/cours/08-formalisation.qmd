...
description: "[Slides](08-formalisation-slides.html)"
format: 
  html: default
  revealjs:
    footer: "[Home](../../index.html) --- [Modélisation](08-formalisation.html)"
    output-file: 08-formalisation-slides.html
categories:
...
:::{.notes}
- (nom, prenom) → nom
- id_joueuse → nom alors (id_joueuse, id_partie) → (nom, id_partie)
:::
...
- Si id_joueuse → (nom, prenom) alors
  - id_joueuse → nom
  - id_joueuse → prenom

Inverse : Union

Auto-détermination : X → X
:::
...
- Un *nom* est atomique
- Une *liste de clubs* n'est pas atomique
...
- participation([id_tournoi, id_joueuse]{.underline}, classement, nom_joueuse, nom_tournoi)
  - *nom_joueuse* dépend uniquement de *id_joueuse*
  - *nom_tournoi* dépend uniquement de *id_tournoi*
...
- joueuse([id_joueuse]{.underline}, code_pays, nom_pays)
- id_joueuse → (code_pays, nom_pays)
- or code_pays → nom_pays donc DF non directe
...
- pays([code_pays]{.underline}, nom_pays)
...
:::{.notes}
- Conceptuel : niveau utilisateur 
  - accents, espaces
  - Entités, Relations, Attributs, Cardinalités
- Logique : compatible avec un SGBD relationnel
  - Entités → table, Attributs → colonnes
...
[cf. Cours BDD, partie associations](./02-bases-de-donnees.qmd#associations)
